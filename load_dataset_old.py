...
class TruckParkingDataset1(Dataset):
    ...
    def load_edge_data(self, path, mapping, src_index_col, dst_index_col, encoders=None):
        edge_idx, edge_attr = load_edge_csv(path, mapping, src_index_col, dst_index_col, names=['SRC_IDX', 'SRC', 'DST_IDX', 'DST', 'DIST'], encoders=encoders, edge_cut=self.edge_cut)
    ...
    def process(self):
        ...
        node_data_list = []
        dfLOC = pd.read_csv(osp.join(self.root, 'data', 'TruckParkingQuery_Location.csv'))
        mapping = {index-1: i for i, index in enumerate(range(1, len(dfLOC)))} # site_idx to idx

        edge_index, edge_attr = self.load_edge_data(link_path, mapping, src_index_col='SRC_IDX', dst_index_col='DST_IDX',
    ...
    def custom_get(self, num_timesteps_in=8, num_timesteps_out=4):
        ...
        for i, j in indices:
            occ = node_data[:, -1, i:i+num_timesteps_in].numpy()
            dayID = node_data[:, 1, i:i+num_timesteps_in].numpy()
            hourID = node_data[:, 2, i:i+num_timesteps_in].numpy()
            features.append(np.stack((occ, hourID, dayID), axis=1))
            # features.append((node_data[:,:,i:i+num_timesteps_in]).numpy())
            target.append((node_data[:, -1, i+num_timesteps_in:j]).numpy())
...
class TruckParkingDataset2(Dataset):
    ...
    def __init__(self, root: str, 
        ...
        self.train_feature = train_feature
        self.root = root
        self.processed_root = osp.join(self.root, 'processed', self.train_feature, 'regional', '0322')
...
class TruckParkingDataset3(Dataset):
    ...
    def load_edge_data(self, path, src_index_col, src_mapping, dst_index_col, dst_mapping, encoders=None):
        edge_idx, edge_attr = load_edge_csv(path, src_index_col, src_mapping, dst_index_col, dst_mapping, names=['SRC_IDX', 'SRC', 'DST_IDX', 'DST', 'DIST'], encoders=encoders)
        return edge_idx, edge_attr
